/**
 * GET /api/admin/v1/metrics/cluster/:identifier
 * 
 * Get aggregated cluster-specific metrics over a time range
...
async function clusterMetricsHandler(
  ...
  try {
    ...
    // Aggregate metrics
    const result = await aggregator.aggregateClusterMetrics(
    ...
    console.error('Cluster metrics aggregation error:', error)
    ...
    if (error.message && error.message.includes('not found')) {
      return res.status(404).json({
        error: {
          code: 'NOT_FOUND',
          message: `Cluster '${req.query.identifier}' not found`,
        },
      })
    }
    ...
    return res.status(500).json({
      error: {
        ...
        message: 'Failed to aggregate cluster metrics',
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  return withClusterAuth(req, res, clusterMetricsHandler, {
