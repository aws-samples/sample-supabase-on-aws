...
const nextConfig = {
    ...
    return [
      {
        ...
        destination: `/api/v1/projects/:path*`,
      },
      {
  ...
  // Limit worker threads during build to reduce memory usage
  experimental: {
    workerThreads: false,
    cpus: 2,
  },
  ...
  webpack: (config, { isServer }) => {
    // Mark AWS SDK packages as external to avoid bundling them during build
    // They will be loaded dynamically at runtime when S3 storage is used
    if (isServer) {
