...
import { IS_PLATFORM, MANAGED_BY } from 'lib/constants'
...
const OrganizationLayoutContent = ({ children }: PropsWithChildren) => {
  ...
  const vercelQuery = useVercelRedirectQuery(
    ...
    {
      enabled: IS_PLATFORM && selectedOrganization?.managed_by === MANAGED_BY.VERCEL_MARKETPLACE,
    }
  )
  ...
  const awsQuery = useAwsRedirectQuery(
    ...
    {
      enabled: IS_PLATFORM && selectedOrganization?.managed_by === MANAGED_BY.AWS_MARKETPLACE,
    }
  )
