...
export function getDatabaseOperations({
  ...
  return {
    async executeSql<T>(_projectRef: string, options: ExecuteSqlOptions) {
      ...
      const { data, error } = await executeQuery<T>({ query, parameters, headers, readOnly, ref: _projectRef })
    ...
    async listMigrations(_projectRef: string) {
      const { data, error } = await listMigrationVersions({ headers, ref: _projectRef })
    ...
    async applyMigration(_projectRef: string, options: ApplyMigrationOptions) {
      ...
      const { error } = await applyAndTrackMigrations({ query, name, headers, ref: _projectRef })
...
export function getDebuggingOperations({
  ...
  return {
    ...
    async getSecurityAdvisors(_projectRef) {
      const { data, error } = await getLints({ headers, ref: _projectRef })
