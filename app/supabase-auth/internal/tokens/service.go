...
type GenerateIDTokenParams struct {
	...
	Context  context.Context  // used for multi-tenant JWT configuration
}
...
func (s *Service) GenerateAccessToken(r *http.Request, tx *storage.Connection, params GenerateAccessTokenParams) (string, int64, error) {
	...
	// Use tenant-specific JWT config if available, otherwise fall back to global
	jwtConfig := &config.JWT
	if tc, ok := tenant.GetTenantFromContext(r.Context()); ok && tc.Config != nil && tc.Config.JWTConfig != nil {
		jwtConfig = tc.Config.JWTConfig
	}

	signed, err := SignJWT(jwtConfig, gotrueClaims)
	if err != nil {
		return "", 0, err
	}
...
func (s *Service) GenerateIDToken(params GenerateIDTokenParams) (string, error) {
	...
	if params.Context != nil {
		if tc, ok := tenant.GetTenantFromContext(params.Context); ok && tc.Config != nil && tc.Config.JWTConfig != nil {
			jwtConfig = tc.Config.JWTConfig
		}
	}

	signingJwk, err := conf.GetSigningJwk(jwtConfig)
	if err != nil {
	...
	signed, err := SignJWT(jwtConfig, claims)
	if err != nil {
		return "", err
	}
