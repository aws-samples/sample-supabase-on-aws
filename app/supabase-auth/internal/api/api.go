...
// getDB returns the appropriate database connection for the request.
// If multi-tenant mode is enabled and a tenant context exists, it returns the tenant's connection.
// Otherwise, it returns the default database connection.
func (a *API) getDB(ctx context.Context) *storage.Connection {
	if tc, ok := tenant.GetTenantFromContext(ctx); ok && tc.Connection != nil {
		return tc.Connection.WithContext(ctx)
	}
	if a.db == nil {
		logrus.Error("getDB called without tenant context and no global database connection")
		return nil
	}
...
func NewAPIWithVersion(globalConfig *conf.GlobalConfiguration, db *storage.Connection, version string, opt ...Option) *API {
	...
	api.tenantManager = tenant.NewManager(&globalConfig.MultiTenant, &globalConfig.JWT)
	api.connectionPool = tenant.NewConnectionPool(api.tenantManager, globalConfig)
	logrus.Info("Multi-tenant support initialized")
	...
	if api.connectionPool != nil {
		r.UseBypass(tenant.Handler(api.connectionPool))
	}
