...
func (a *API) loadExternalStateFromJWT(ctx context.Context, db *storage.Connection, state string) (context.Context, error) {
	...
	if tc, ok := tenant.GetTenantFromContext(ctx); ok && tc.Config != nil && tc.Config.JWTConfig != nil {
		jwtConfig = tc.Config.JWTConfig
	}

	claims := ExternalProviderClaims{}
	p := jwt.NewParser(jwt.WithValidMethods(jwtConfig.ValidMethods))
	_, err := p.ParseWithClaims(state, &claims, func(token *jwt.Token) (interface{}, error) {
		if kid, ok := token.Header["kid"]; ok {
			if kidStr, ok := kid.(string); ok {
				key, err := conf.FindPublicKeyByKid(kidStr, jwtConfig)
				if err != nil {
					return nil, err
				}
