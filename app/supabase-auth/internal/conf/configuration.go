...
type GlobalConfiguration struct {
	...
	MultiTenant  MultiTenantConfiguration  `json:"multi_tenant" split_words:"true"`
}
...
// BuildJWTConfigFromSecret builds a complete JWTConfiguration (including Keys) from
// a base config and an HS256 secret. The base config is value-copied so that fields
// like Exp, Issuer, Aud, AdminRoles, etc. are inherited.
func BuildJWTConfigFromSecret(base JWTConfiguration, secret string) (JWTConfiguration, error) {
	jwtConfig := base // value copy – inherits Exp/Issuer/Aud/ValidMethods etc.
	jwtConfig.Secret = secret
	jwtConfig.Keys = make(JwtKeysDecoder)
	...
	if jwtConfig.KeyID != "" {
		if err := privKey.Set(jwk.KeyIDKey, jwtConfig.KeyID); err != nil {
			return jwtConfig, err
		}
	}
	...
	jwtConfig.Keys[jwtConfig.KeyID] = JwkInfo{PublicKey: pubKey, PrivateKey: privKey}
	jwtConfig.ValidMethods = []string{jwt.SigningMethodHS256.Name}
...
// MultiTenantConfiguration holds the configuration for multi-tenant support
type MultiTenantConfiguration struct {
	TenantManagerURL string        `json:"tenant_manager_url" split_words:"true"`
	TenantManagerKey string        `json:"tenant_manager_key" split_words:"true"`
	CacheTTL         time.Duration `json:"cache_ttl" split_words:"true" default:"5m"`
	MaxConnections   int           `json:"max_connections" split_words:"true" default:"100"`
	MaxIdleTime      time.Duration `json:"max_idle_time" split_words:"true" default:"10m"`
}
