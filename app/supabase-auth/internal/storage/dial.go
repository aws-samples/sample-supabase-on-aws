...
// DialWithURL creates a connection using a specific database URL
// This is used for multi-tenant support where each tenant has their own database.
// The URL is parsed into individual components so that pop generates the final
// connection string via OptionsString(), which ensures search_path=auth is
// included and applied by pgx as a RuntimeParam for every pooled connection.
func DialWithURL(ctx context.Context, databaseURL string, config *conf.GlobalConfiguration) (*Connection, error) {
	u, err := url.Parse(databaseURL)
	if err != nil {
		return nil, errors.Wrap(err, "parsing tenant database URL")
	}
	...
	opts := map[string]string{
		...
		"search_path": config.DB.Namespace,
	}
	if config.DB.HealthCheckPeriod != time.Duration(0) {
		opts["pool_health_check_period"] = config.DB.HealthCheckPeriod.String()
	}
	if config.DB.ConnMaxIdleTime != time.Duration(0) {
		opts["pool_max_conn_idle_time"] = config.DB.ConnMaxIdleTime.String()
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "opening tenant database connection")
	}

	if err := db.Open(); err != nil {
		return nil, errors.Wrap(err, "checking tenant database connection")
	}
