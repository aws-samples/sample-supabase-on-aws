...
func migrate(cmd *cobra.Command, args []string) {
	...
	// Ensure the namespace schema exists before running migrations.
	// The migrator creates schema_migrations in the current search_path,
	// which requires the schema to already exist.
	if ns := globalConfig.DB.Namespace; ns != "" {
		if err := db.RawQuery("CREATE SCHEMA IF NOT EXISTS " + ns).Exec(); err != nil {
			log.Fatalf("%+v", errors.Wrap(err, "creating namespace schema"))
		}
		log.Infof("Ensured schema %q exists", ns)
	}
